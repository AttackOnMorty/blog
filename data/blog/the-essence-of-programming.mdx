...
There are two important points I want to elaborate on. One is the role of control in the algorithm, and the other is how to write good programs.

#### The Role of Control on Algorithm
...
It means the `Control only affects the efficiency of the Algorithm. It doesn't change the Logic`. For example, we have a simple problem:
...
We can see we mixed the logic and control in the function. It will become more and more complex as the business logic grows.

Logic is the lower limit of program complexity. Then, to control the program, we need to create a lot of control codes, so the mixture of Logic + Control forms the final complexity of the program.

Actually, we can create a DSL ([Domain-specific language](https://en.wikipedia.org/wiki/Domain-specific_language)) + a DSL parser to separate the logic and control.
...
In this way, DSL (rules) describes the logic, and the `validate` method becomes the control. The code now looks more readable and maintainable.
...
All programming languages are trying to do the following things:
...
- **Control** only affects the efficiency of **Algorithm**. It doesn't change the **Logic**.
